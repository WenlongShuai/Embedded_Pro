...
void atk_ms53l1m_iic_start(uint8_t dir)
{
	switch(dir)
	{
		...
		case 4:
			...
			ATK_MS53L1M_IIC_SDA_LEFTSIDE(1);
			ATK_MS53L1M_IIC_SCL_LEFTSIDE(1);
			atk_ms53l1m_iic_delay();
			ATK_MS53L1M_IIC_SDA_LEFTSIDE(0);
			atk_ms53l1m_iic_delay();
			ATK_MS53L1M_IIC_SCL_LEFTSIDE(0);
			atk_ms53l1m_iic_delay();
			#endif
			break;
		case 5:
			#if ATK_MS53L1M_RIGHTSIDE_ON
			ATK_MS53L1M_IIC_SDA_RIGHTSIDE(1);
			ATK_MS53L1M_IIC_SCL_RIGHTSIDE(1);
			atk_ms53l1m_iic_delay();
			ATK_MS53L1M_IIC_SDA_RIGHTSIDE(0);
			atk_ms53l1m_iic_delay();
			ATK_MS53L1M_IIC_SCL_RIGHTSIDE(0);
			atk_ms53l1m_iic_delay();
			#endif
			break;
	}
...
uint8_t atk_ms53l1m_iic_wait_ack(uint8_t dir)
{
	...
	switch(dir)
	{
		...
		case 4:
			...
			while (ATK_MS53L1M_IIC_READ_SDA_LEFTSIDE())
			{
					waittime++;
					
					if (waittime > 250)
					{
							atk_ms53l1m_iic_stop(DIR_LEFTSIDE);
							rack = 1;
							break;
					}
			}
		...
		case 5:
			...
			while (ATK_MS53L1M_IIC_READ_SDA_RIGHTSIDE())
			{
					waittime++;
					
					if (waittime > 250)
					{
							atk_ms53l1m_iic_stop(DIR_RIGHTSIDE);
							rack = 1;
							break;
					}
			}
...
void atk_ms53l1m_iic_send_byte(uint8_t dir, uint8_t dat)
{
	...
	switch(dir)
	{
		...
		case 4:
			...
			for (t=0; t<8; t++)
			{
					ATK_MS53L1M_IIC_SDA_LEFTSIDE((dat & 0x80) >> 7);
					atk_ms53l1m_iic_delay();
					ATK_MS53L1M_IIC_SCL_LEFTSIDE(1);
					atk_ms53l1m_iic_delay();
					ATK_MS53L1M_IIC_SCL_LEFTSIDE(0);
		...
		case 5:
			...
			for (t=0; t<8; t++)
			{
					ATK_MS53L1M_IIC_SDA_RIGHTSIDE((dat & 0x80) >> 7);
					atk_ms53l1m_iic_delay();
					ATK_MS53L1M_IIC_SCL_RIGHTSIDE(1);
					atk_ms53l1m_iic_delay();
					ATK_MS53L1M_IIC_SCL_RIGHTSIDE(0);
	...
	switch(dir)
	{
		...
		case 4:
			...
			for (i = 0; i < 8; i++ )
			{
					...
					if (ATK_MS53L1M_IIC_READ_SDA_LEFTSIDE())
					{
							dat++;
					}
			...
			if (ack == 0)
			{
					atk_ms53l1m_iic_nack(DIR_LEFTSIDE);
			}
			else
			{
					atk_ms53l1m_iic_ack(DIR_LEFTSIDE);
			}
			#endif
			break;
		case 5:
			...
			for (i = 0; i < 8; i++ )
			{
					...
					if (ATK_MS53L1M_IIC_READ_SDA_RIGHTSIDE())
					{
							dat++;
					}
			...
			if (ack == 0)
			{
					atk_ms53l1m_iic_nack(DIR_RIGHTSIDE);
			}
			else
			{
					atk_ms53l1m_iic_ack(DIR_RIGHTSIDE);
			}
			#endif
			break;
	}
...
void atk_ms53l1m_iic_init(uint8_t dir)
{
	...
	switch(dir)
	{
		...
		case 4:
			...
			/* 使能SCL、SDA引脚GPIO的时钟 */
			ATK_MS53L1M_IIC_SCL_GPIO_CLK_ENABLE_LEFTSIDE();
			ATK_MS53L1M_IIC_SDA_GPIO_CLK_ENABLE_LEFTSIDE();
			
			/* 初始化SCL引脚 */
			gpio_init_struct.GPIO_Pin    = ATK_MS53L1M_IIC_SCL_GPIO_PIN_LEFTSIDE; /* SCL引脚 */
			gpio_init_struct.GPIO_Mode   = GPIO_Mode_OUT;          /* 推挽输出 */
			gpio_init_struct.GPIO_Speed  = GPIO_Speed_100MHz;         /* 高速 */
			gpio_init_struct.GPIO_OType = GPIO_OType_PP;
			gpio_init_struct.GPIO_PuPd = GPIO_PuPd_UP;
			GPIO_Init(ATK_MS53L1M_IIC_SCL_GPIO_PORT_LEFTSIDE, &gpio_init_struct);
			
			/* 初始化SDA引脚 */
			gpio_init_struct.GPIO_Pin = ATK_MS53L1M_IIC_SDA_GPIO_PIN_LEFTSIDE; /* SDA引脚 */
			gpio_init_struct.GPIO_OType = GPIO_OType_OD;          /* 开漏输出 */
			GPIO_Init(ATK_MS53L1M_IIC_SDA_GPIO_PORT_LEFTSIDE, &gpio_init_struct);
		...
		case 5:
			...
			/* 使能SCL、SDA引脚GPIO的时钟 */
			ATK_MS53L1M_IIC_SCL_GPIO_CLK_ENABLE_RIGHTSIDE();
			ATK_MS53L1M_IIC_SDA_GPIO_CLK_ENABLE_RIGHTSIDE();
			
			/* 初始化SCL引脚 */
			gpio_init_struct.GPIO_Pin    = ATK_MS53L1M_IIC_SCL_GPIO_PIN_RIGHTSIDE; /* SCL引脚 */
			gpio_init_struct.GPIO_Mode   = GPIO_Mode_OUT;          /* 推挽输出 */
			gpio_init_struct.GPIO_Speed  = GPIO_Speed_100MHz;         /* 高速 */
			gpio_init_struct.GPIO_OType = GPIO_OType_PP;
			gpio_init_struct.GPIO_PuPd = GPIO_PuPd_UP;
			GPIO_Init(ATK_MS53L1M_IIC_SCL_GPIO_PORT_RIGHTSIDE, &gpio_init_struct);
			
			/* 初始化SDA引脚 */
			gpio_init_struct.GPIO_Pin = ATK_MS53L1M_IIC_SDA_GPIO_PIN_RIGHTSIDE; /* SDA引脚 */
			gpio_init_struct.GPIO_OType = GPIO_OType_OD;          /* 开漏输出 */
			GPIO_Init(ATK_MS53L1M_IIC_SDA_GPIO_PORT_RIGHTSIDE, &gpio_init_struct);
