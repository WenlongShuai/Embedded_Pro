...
/*******************************************
MV	MX	MY	显示效果												适用场景
0	 0	0	    原始方向（默认，竖屏）					标准竖屏显示
0	 1	0			水平镜像（左右翻转）						抵消屏幕物理左右装反
0	 0	1			垂直镜像（上下翻转）						抵消屏幕物理上下装反
0	 1	1			180 度旋转（水平 + 垂直镜像）	屏幕物理倒装 180 度
1	 0	0			顺时针旋转 90 度（横屏）				横屏显示（宽 320× 高 240）
1	 1	0			顺时针旋转 90 度 + 水平镜像		特殊横屏场景
1	 0	1			逆时针旋转 90 度（横屏）				另一种横屏显示方式
1	 1	1			逆时针旋转 90 度 + 水平镜像		特殊横屏场景
********************************************/
...
//	Lcd_WriteIndex(0x01);
	...
	Lcd_WriteIndex(0x11);  //Sleep Out.This command turns off sleep mode.
	Lcd_WriteData(0x00); 

	Lcd_WriteIndex(0xCF);   //Power control B.
	Lcd_WriteData(0X00); 
	Lcd_WriteData(0XC1); 
	Lcd_WriteData(0X30);   //Discharge path enable. Enable high for ESD protection
  ...
  Lcd_WriteIndex(0x36);  
	#if USE_HORIZONTAL
		 Lcd_WriteData(0x60);//横屏参数
	#else
		 Lcd_WriteData(0x00); //竖屏参数
	#endif
...
返回值：无
*************************************************/
void Lcd_Clear_Area(uint16_t x, uint16_t y, uint16_t width, uint16_t height, uint16_t Color)               
{	
	unsigned int i;
