...
{
#if !IIC_SELECT
	Hardware_Write_longByte(1, OLED_ID, OLED_WR_DATA, 1, &data);
#else
	Software_Write_longByte(1, OLED_ID, OLED_WR_DATA, 1, &data);
#endif
}
...
static void Oled_Write_Cmd(uint8_t cmd)
{
	#if !IIC_SELECT
	uint8_t ret = 0;
	ret = Hardware_Write_longByte(1, OLED_ID, OLED_WR_CMD, 1, &cmd);
	printf("--->%d\r\n",ret);
	#else
	Software_Write_longByte(1, OLED_ID, OLED_WR_CMD, 1, &cmd);
	#endif
}
