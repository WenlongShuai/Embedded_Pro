...
volatile float fAcc[3] = {0.0f}, fGyro[3] = {0.0f}, fMag[3] = {0.0f}, fAngle[3] = {0.0f};
...
//int32_t WitI2cWriteFun(uint8_t ucAddr, uint8_t ucReg, uint32_t uiLen, uint8_t *p_ucVal)
//{
//		Software_iic_start(0);
//		Software_iic_send_byte(0, ucAddr);
//		if(Software_iic_wait_ack(0) == SUCCESS) return 0;
//		Software_iic_send_byte(0, ucReg);
//		if(Software_iic_wait_ack(0) == SUCCESS) return 0;
//		for(uint32_t i = 0; i < uiLen; i++)
//		{
//				Software_iic_send_byte(0, p_ucVal[i]); 
//				if(Software_iic_wait_ack(0) == SUCCESS) return 0;
//		}
//		Software_iic_stop(0);
//		return 1;
...
//int32_t WitI2cReadFun(uint8_t ucAddr, uint8_t ucReg, uint32_t uiLen, uint8_t *p_ucVal)
//{
//		Software_iic_start(0);
//		Software_iic_send_byte(0, ucAddr);
//		if(Software_iic_wait_ack(0) == SUCCESS)return 0;
...
//	
//		Software_iic_start(0);
//		Software_iic_send_byte(0, ucAddr+1);
//		for(uint32_t i = 0; i < uiLen; i++)
//		{
//				if(i != uiLen-1) p_ucVal[i] = Software_iic_read_byte(0, 1);   //  ask
//				else p_ucVal[i] = Software_iic_read_byte(0, 0); //last byte no ask
//		}
//		Software_iic_stop(0);
//		return 1;
//}
...
int32_t WitWriteReg(uint32_t uiReg, uint16_t usData)
{
    ...
    switch(s_uiProtoclo)
    {
        ...
        case WIT_PROTOCOL_I2C:
            ...
            ucBuff[1] = usData >> 8;
			if(p_WitI2cWriteFunc(s_ucAddr, uiReg, ucBuff, 2) != 0)
			{
...
int32_t WitReadReg(uint32_t uiReg, uint32_t uiReadNum)
{
    ...
    switch(s_uiProtoclo)
    {
        ...
        case WIT_PROTOCOL_I2C:
              ...
              if(p_WitI2cReadFunc(s_ucAddr, uiReg, s_ucWitDataBuff, usTemp) == 0)
              {
...
void CmdProcess(void)
{
	switch(s_cCmd)
	{
		...
		case 'B':	
			...
			{
				if(s_uiProtoclo == WIT_PROTOCOL_NORMAL)
				{
					JY901P_USART_Init(c_uiBaud[WIT_BAUD_115200]);											
				}
			}				
			break;
		case 'b':	
			...
			else 
			{
				if(s_uiProtoclo == WIT_PROTOCOL_NORMAL)
				{
					JY901P_USART_Init(c_uiBaud[WIT_BAUD_9600]);											
				}
			}
			break;
		case 'R':	
...
int32_t WitI2cWriteFun(uint8_t ucAddr, uint8_t ucReg, uint8_t *p_ucVal, uint32_t uiLen)
{
	return Software_Write_longByte(0, ucAddr, ucReg, uiLen, p_ucVal);
}
	
	
int32_t WitI2cReadFun(uint8_t ucAddr, uint8_t ucReg, uint8_t *p_ucVal, uint32_t uiLen)
{
	return Software_Read_Byte(0, ucAddr, ucReg, uiLen, p_ucVal);
}


/* ³õÊ¼»¯WitÄ£¿é */
void WitModuleInit(void)
{
	#if defined(USARTMODE)
	JY901P_USART_Init(115200);
	WitInit(WIT_PROTOCOL_NORMAL, 0x50);
	WitSerialWriteRegister(JY901P_SendString);
	...
	WitI2cFuncRegister(WitI2cWriteFun, WitI2cReadFun);
	WitRegisterCallBack(SensorDataUpdata);
	WitDelayMsRegister(Delayms);
	AutoScanSensor();
	#endif
