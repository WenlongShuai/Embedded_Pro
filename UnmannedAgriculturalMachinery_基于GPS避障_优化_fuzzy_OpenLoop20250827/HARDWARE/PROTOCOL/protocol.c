...
uint8_t recv_buf[PROT_FRAME_LEN_RECV];
...
static int32_t recvbuf_find_header(uint8_t *buf, uint16_t ring_buf_len, uint16_t start, uint16_t len)
{
    ...
    for (i = 0; i < (len - 3); i++)
    {
        if (((buf[(start + i + 0) % ring_buf_len] <<  0) |
             ...
             (buf[(start + i + 3) % ring_buf_len] << 24)) == FRAME_HEADER_PID)
        {
  ...
  while(1)
  {
    ...
    switch (cmd_type)
    {
      ...
      case SET_TARGET_CMD:
      {
        ...
        set_PID_Target_A(actual_temp);    //设置目标值
				#endif
      }
      break;
...
void set_computer_value(uint8_t cmd, uint8_t ch, void *data, uint8_t num)
{
  ...
  set_packet.head = FRAME_HEADER_PID;     // 包头 0x59485A53
  ...
  sum = check_sum(sum, (uint8_t *)data, num);                           // 计算参数校验和
	...
	Usart_SendString((uint8_t *)&set_packet, sizeof(set_packet));// 发送数据头
	Usart_SendString((uint8_t *)data, num);// 发送参数
	Usart_SendString((uint8_t *)&sum, sizeof(sum));// 发送校验和
}
