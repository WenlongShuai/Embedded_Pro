...
// 软件 IIC 定义
#define SEEKFREE_SCL	D10															// 定义SCL引脚  可任意更改为其他IO
#define SEEKFREE_SDA	D11															// 定义SDA引脚  可任意更改为其他IO
...
void	simiic_write_reg		(uint8 dev_add, uint16 reg, uint8 dat);
void	simiic_write_regs		(uint8 dev_add, uint16 reg, uint8 *dat, uint8 num);
uint8	simiic_read_reg			(uint8 dev_add, uint16 reg, IIC_type type);
void	simiic_read_regs		(uint8 dev_add, uint16 reg, uint8 *dat_add, uint8 num, IIC_type type);
